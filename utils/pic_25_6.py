...
class MetricsVisualizationTool:
    ...
    def __init__(self, data_dir: str = "./output", output_dir: str = "./charts_simple", 
                 ...
                 display_start_time: str = None, display_end_time: str = None,Range = 'all',is_normlize = False):
        """
        初始化工具
    ...
    def load_csv_files(self, csv_file_path: str = None) -> List[Tuple[str, pd.DataFrame]]:
        ...
        try:
            ...
            for metric_col in metric_columns:
                ...
                if self.is_normlize:
                    metric_df['value'] = (metric_df['value'] - metric_df['value'].min()) / (metric_df['value'].max() - metric_df['value'].min())
    ...
    def create_single_figure(self, csv_files: List[Tuple[str, pd.DataFrame]], fig_num: int):
        ...
        # 保存图片
        output_path = os.path.join(self.output_dir, f'{fig_num}_{self.range}.png')
...
def main():
    ...
    # fault_start =  pd.to_datetime('2022/5/9 8:32')
    # fault_end = fault_start + pd.Timedelta(minutes=10)
    # display_start = fault_start - pd.Timedelta(minutes=20)
    # display_end = fault_end + pd.Timedelta(minutes=5)
    # fault_start = fault_start.strftime("%Y-%m-%d %H:%M:%S")
    # fault_end = fault_end.strftime("%Y-%m-%d %H:%M:%S")
