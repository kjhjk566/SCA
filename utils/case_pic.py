...
class MetricsVisualizationTool:
    ...
    def __init__(self, data_dir: str = "./output", output_dir: str = "./charts_simple", 
                 ...
                 display_start_time: str = None, display_end_time: str = None, data_df: pd.DataFrame = None):
        """
        初始化工具
        
        Args:
            data_dir: 数据目录（用于CSV文件模式）
            output_dir: 图表输出目录
            ...
            display_end_time: 显示结束时间
            data_df: DataFrame数据源（第一列为时间戳，后续列为指标）
        """
        ...
        self.data_df = data_df  # DataFrame数据源
    ...
    def load_dataframe(self, df: pd.DataFrame = None) -> List[Tuple[str, pd.DataFrame]]:
        """
        加载DataFrame数据，第一列是时间，后面每列是一个指标
        
        Args:
            df: DataFrame数据，如果为None则使用self.data_df
        ...
        # 如果没有传入DataFrame，使用初始化时的data_df
        if df is None:
            df = self.data_df
        ...
        try:
            ...
            if 'time' in time_column.lower():
                # 如果是Unix时间戳（秒）
                df_copy['datetime'] = pd.to_datetime(df_copy[time_column], unit='s') + pd.Timedelta(hours=8)
            else:
                # 尝试直接解析时间格式
                df_copy['datetime'] = pd.to_datetime(df_copy[time_column]) + pd.Timedelta(hours=8)
            print(f"Parsed datetime column: {df_copy['datetime'].head()}")
            
            # 获取所有指标列（除了时间列）
            metric_columns = [col for col in df_copy.columns if col != time_column and col != 'datetime']
            ...
            for metric_col in metric_columns:
                ...
                metric_df = df_copy[['datetime', metric_col]].copy()
            ...
            print(f"Successfully loaded {len(csv_files)} metrics from DataFrame")
    ...
    def create_charts(self, csv_file_path: str = None, df: pd.DataFrame = None):
        """
        创建所有图表
        
        Args:
            csv_file_path: CSV文件路径，如果为None则使用data_dir（CSV文件模式）
            df: DataFrame数据源，如果提供则优先使用（DataFrame模式）
        """
        # 根据输入参数决定使用哪种数据加载方式
        if df is not None:
            # DataFrame模式
            csv_files = self.load_dataframe(df)
            data_source = "DataFrame"
        elif self.data_df is not None:
            # 使用初始化时提供的DataFrame
            csv_files = self.load_dataframe()
            data_source = "DataFrame (from init)"
        else:
            # CSV文件模式
        ...
        if not csv_files:
            print(f"No valid metrics found in {data_source}")
            return
    ...
    def generate_summary_report(self, csv_file_path: str = None, df: pd.DataFrame = None):
        """
        生成分析报告
...
def main():
    ...
    fault_start = '2025-05-20 02:10:13'
    fault_end = '2025-05-20 02:20:13'
    ...
    case_id = 0
    import pickle
    with open(f"/home/kuangjunhua/research/data/aiops25/{day}/case_20min_data.pkl", "rb") as f:
        data = pickle.load(f)
    df_data = data[case_id][0]

    
    
    
    # 方式2: 使用DataFrame模式（新增方式）
    print("\n=== DataFrame模式 ===")
    # 从CSV文件加载数据作为示例
    
    
    tool2 = MetricsVisualizationTool(
        output_dir=f"/home/kuangjunhua/research/data/aiops25/20/chart/",
        fault_start_time=None,
        fault_end_time=None,
        metric_name=['k8s-master1'],
        display_start_time=None,
        display_end_time=None,
        data_df=df_data  # 直接传入DataFrame
    )
    # 生成图表
    tool2.create_charts()  # 使用初始化时的DataFrame
    tool2.generate_summary_report()
    
    
    
    print("Visualization completed!")
    
    print("DataFrame模式图表保存在:", tool2.output_dir)
    
    if fault_start and fault_end:
        print(f"故障时间段高亮: {fault_start} 到 {fault_end}")
